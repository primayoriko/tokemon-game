...
calc_health :-  
            ...
            current_tokemon1(X1,E,Y1,_), current_tokemon2(X2,F,Y2,_),
            retractall(current_tokemon1(_,_,_,_)),retractall(current_tokemon2(_,_,_,_)),
            (E < 0 -> (assert(current_tokemon1(X1,0,Y1,_)), assert(current_tokemon1(X1,F,Y1,_))); 
             (assert(current_tokemon1(X1,E,Y1,_)), assert(current_tokemon1(X1,F,Y1,_)))),
            (F < 0 ->(assert(current_tokemon1(X1,E,Y1,_)), assert(current_tokemon1(X1,0,Y1,_)));
            (assert(current_tokemon1(X1,E,Y1,_)), assert(current_tokemon1(X1,F,Y1,_)))),
            tulis_battle,
            see_result.
...
win :-
        ...
        retractall(current_tokemon1(_,_,_,_)),
        write('Do you want to capture the pokemon?, walk away if you dont'), nl, !.

lose :-
        maxInventory(Z), Z>1,
        current_tokemon1(A,_,_,_), drop(A),
        retractall(current_tokemon1(_,_,_,_)),
        write('Tokemonmu telah mati, pilih tokemon lain!'),nl,
        !.
...
loseTheGame :-
        write('Sayang sekali, semua tokemonmu telah mati.'),nl,
        write('Karena kamu gagal, kamu juga mati.'),
        !.
...
capture :-
        maxInventory(Z), Z =:=6,
        write('Inventorymu penuh!'),nl,write('Drop salah satu tokemonmu lalu capture lagi!'),nl,!.
...
specialattack :-
            ...
            tokemon(Z,_,_,A,W,C,X), current_tokemon1(X,E,G,H), tokemon(_,_,B,_,_,D,Y), current_tokemon2(Y,F,I,J),
            C == "grass" , D == "water",
            write(Z), write(" use "), write(W), write("!"),nl,
            write("Wow! Super Effective."), nl,
            V is A*15/10, E2 is E-B, F2 is F-V,
            ((E2 >0, F2 > 0) ->
            (retract(current_tokemon1(X,E,G,H)),retract(current_tokemon2(Y,F,I,J)),
            asserta(current_tokemon1(X,E2,G,H)),asserta(current_tokemon2(Y,F2,I,J)),tulis_battle) ;(
            ((E2 > 0 , F2 =< 0) ->  win;(
                    (E2 =< 0, F2 > 0) -> (write('u lose'),nl) ;write('seri'))
            ))),
            !.
...
specialattack :- 
            ...
            V is A*5/10, E2 is E-B, F2 is F-V,
            ((E2 >0, F2 > 0) ->
            (retract(current_tokemon1(X,E,G,H)),retract(current_tokemon2(Y,F,I,J)),
            asserta(current_tokemon1(X,E2,G,H)),asserta(current_tokemon2(Y,F2,I,J)),tulis_battle) ;(
            ((E2 > 0 , F2 =< 0) ->  win;(
...
specialattack :-
            /* normal condtion */
            tokemon(Z,_,_,A,W,C,X), current_tokemon1(X,E,G,H), tokemon(_,_,B,_,_,C,Y), current_tokemon2(Y,F,I,J),
            write(Z), write(" use "), write(W), write("!"),nl,
            E2 is E-B, F2 is F-A,
            ((E2 >0, F2 > 0) ->
            (retract(current_tokemon1(X,E,G,H)),retract(current_tokemon2(Y,F,I,J)),
